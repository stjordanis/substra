...
def test_tuples_execution_on_same_node():
    ...
    factory = AssetsFactory("test_local")
    client = substra.Client(profile_name="default", backend="local",)

    default_dataset_dict = factory.create_dataset().dict(exclude_none=True)
    default_dataset = client.add_dataset(default_dataset_dict)

    # create train data samples
    for i in range(4):
        spec = factory.create_data_sample(datasets=[default_dataset], test_only=False).dict(exclude_none=True)
        client.add_data_sample(spec)

    # create test data sample
    spec = factory.create_data_sample(datasets=[default_dataset], test_only=True).dict(exclude_none=True)
    test_data_sample = client.add_data_sample(spec)

    default_objective_dict = factory.create_objective(
        dataset=default_dataset, data_samples=[test_data_sample]
    ).dict(exclude_none=True)
    default_objective = client.add_objective(default_objective_dict)

    spec = factory.create_algo().dict(exclude_none=True)
